<template>
  <section class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden flex flex-col justify-between">
        <div>
          ...
          <div class="mt-2 flex items-baseline space-x-2">
            <span class="text-2xl font-bold text-slate-900">¥{{ formatMoney(displayTotalAnnualPremium) }}</span>
        ...
        <div class="absolute right-3 -bottom-2 text-sky-50 text-6xl pointer-events-none -z-0">
      ...
      <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden flex flex-col justify-between">
        <div>
          ...
          <div class="mt-2 flex items-baseline space-x-2">
            <span class="text-2xl font-bold text-slate-900">¥{{ formatMoney(totalPaidAmount) }}</span>
          </div>
          <!-- 双轨细分拆解 -->
          <div class="mt-2.5 flex items-center gap-1.5 text-xs flex-wrap">
        ...
        <div class="absolute right-3 -bottom-2 text-indigo-50 text-6xl pointer-events-none -z-0">
      ...
      <div
        ...
        class="bg-white p-5 rounded-2xl border transition cursor-pointer relative overflow-hidden group hover:shadow-md border-slate-200/80 hover:border-sky-300"
      >
        ...
        <div class="mt-2 flex items-baseline space-x-2">
          <span class="text-2xl font-extrabold text-sky-700">{{ nearDueCount > 0 ? nearDueCount : upcomingCount }}</span>
      ...
      <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden">
        ...
        <div class="mt-2 flex items-baseline space-x-2">
          <span class="text-2xl font-bold text-sky-600">{{ activePolicies.length }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div class="flex justify-between items-center mb-4">
          <div>
            <h3 class="text-base font-bold text-slate-900">历年保费支出走势 (2018 - 2026)</h3>
            <p class="text-xs text-slate-500">从早期单人参保到全家成熟保障体系的费用变动</p>
          </div>
          <span class="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg font-medium">柱状趋势</span>
        </div>
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col">
        <div class="mb-4">
          <h3 class="text-base font-bold text-slate-900">家庭成员年保费占比</h3>
          <p class="text-xs text-slate-500">当前各成员商业保费贡献比例</p>
        </div>
        <div class="h-52 relative flex-1">
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="text-base font-bold text-slate-900">家庭成员保障档案概览</h3>
          <p class="text-xs text-slate-500 mt-0.5">点击卡片可快速筛选该成员名下的所有保单</p>
        </div>
        <button
          @click="$emit('open-members')"
          class="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-slate-100 hover:bg-sky-50 text-slate-700 hover:text-sky-700 rounded-xl text-xs font-medium transition cursor-pointer"
          title="管理家庭成员档案与社保信息"
        >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div
          ...
          class="p-4 rounded-xl border border-slate-200 hover:border-sky-300 hover:shadow-md transition cursor-pointer bg-slate-50/50 flex flex-col justify-between"
        >
          <div class="flex items-center justify-between">
          ...
          <div class="mt-3">
            <div class="text-xs text-slate-500">年度预算</div>
            <div class="text-lg font-extrabold text-sky-700">¥{{ formatMoney(val) }}</div>
          </div>
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          <div class="flex items-center space-x-2">
            <h3 class="text-base font-bold text-slate-900">家庭爱车保障与资产概览</h3>
